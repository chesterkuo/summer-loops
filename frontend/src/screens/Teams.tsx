  ...
  const addMember = async () => {
    ...
    if (result.data) {
      if (result.data.invited) {
        // Invite was sent to non-registered user
        setAddMemberSuccess(`Invitation sent to ${result.data.email}. They'll be added when they sign up.`);
        setNewMemberEmail('');
        setNewMemberRole('member');
      } else {
        // User was added directly
        const [teamResult, contactsResult] = await Promise.all([
          teamsApi.get(selectedTeam.id),
          teamsApi.getContacts(selectedTeam.id),
        ]);
        if (teamResult.data) setSelectedTeam(teamResult.data);
        if (contactsResult.data) setTeamContacts(contactsResult.data);
            ...
            <div className="bg-[#2C3435] w-full max-w-sm rounded-2xl p-5 border border-white/10 shadow-2xl">
              ...
              {addMemberSuccess && (
                <div className="mt-3 p-3 bg-green-900/20 border border-green-800/30 rounded-lg">
                  <p className="text-xs text-green-400">{addMemberSuccess}</p>
                </div>
              )}

              <div className="flex gap-3 mt-6">
                <button
                  onClick={() => { setShowAddMemberModal(false); setAddMemberError(''); setAddMemberSuccess(''); }}
